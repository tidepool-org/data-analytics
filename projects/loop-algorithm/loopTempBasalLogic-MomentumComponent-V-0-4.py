...
cgmTimes =  [5,    60, 120, 180, 240, 350, 355, 360]
cgmValues = np.array([100, 140, 180, 200, 210, 100, 103, 106])
...
# new format
figureName = "momentum-effect-example"
...
# plot slope
ax.plot(simulatedTime[-3:], simulatedCgm[-3:], linestyle="-.", color="#3C31FF", lw=3, label="Momentum Slope")
...
ax.plot(mTime, mCgm, linestyle="--", color="#3C31FF", lw=3, label="Predicted Glucose (Momentum Effect Only)")

# plot predicted cgm from momentum
ax.plot(pred20minTime, pred20minCgm, linestyle="--", color="#AC37f0", lw=3, label="Predicted Glucose (Other Effects Only)")

# plot predicted cgm from momentum
ax.plot(predictedTime, predictedCgm, linestyle="--", color="#31B0FF", lw=3, label="Predicted Glucose (Blended)")
...
ax = common_figure_elements(ax, xLabel, yLabel, figureFont, labelFontSize, tickLabelFontSize, coord_color, yLabel_xOffset=-295)
...
# format the legend
leg = ax.legend(handles, labels, scatterpoints=3, edgecolor="black", loc=2)
