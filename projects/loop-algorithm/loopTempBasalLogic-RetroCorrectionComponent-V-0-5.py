...
cgmValues = np.array([100, 140, 180, 200, 210, 195, 205, 200]) - 35
...
# set up the retrospective correction
...
cgmValues = np.array([177, 178, 181])
...
## %% get the retrospective correction with decay
rcTime = np.arange(5, 65, 5)
rcCgm = -bgVel * (1 - ((rcTime - 5)/55))

predictedTime = np.append(360, rcTime+360)
predictedCgm = np.append(simulatedCgm[-1], simulatedCgm[-1] + np.cumsum(rcCgm))

# new format
figureName = "rc-effect-example"
...
#ax.plot(predictedTime, pred30minCgm,
#        marker='v', markersize=14, color="#f09a37",
#        ls="None", label="%d Unit of Insulin Delivered" % insulinAmount)

# plot predicted cgm
#ax.plot(pred30minTime, pred30minCgm, linestyle="-.", color="#3175FF", lw=2, label="Retrospective Forecast")
ax.scatter(pred30minTime, pred30minCgm, s=10, color="#3175FF", label="Retrospective Forecast")
...
# plot predicted cgm
ax.plot(predictedTime, predictedCgm, linestyle="--", color="#3175FF", lw=2, label="Predicted Glucose (RC Effect Only)")
...
ax = common_figure_elements(ax, xLabel, yLabel, figureFont, labelFontSize, tickLabelFontSize, coord_color, yLabel_xOffset=-230)
...
ax.set_xlim([240, max(predictedTime) + 30])
