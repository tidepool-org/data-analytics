...
# import data
basalData = pd.read_csv("basalLite.csv", low_memory=False)
#bolusData = pd.read_csv("bolusExample.csv", low_memory=False)
cgmData = pd.read_csv("cgmLite.csv", low_memory=False)
...
for dy in dayStart[1:-1]:

    # % grab data 6 hours after the last bolus

    figureClass = "%s-est-isf-br-v1-" % dy
    ...
    if ((len(filteredBasalData) > 0) & (filteredBasalData.invalidData.sum() == 0)):
        ...
        for bIndex in range(len(filteredBasalData)):
            ...
            if durationMilliSeconds > 0:
                ...
                if bIndex < (len(filteredBasalData) - 1):
        ...
        #leahISF = 40
        #ax.plot(basalInsulinEffect.hoursSinceBasalDelivery,
        #        basalInsulinEffect.normalizedDeltaGlucoseEffect * leahISF,
        #        lw=4, ls="--", color="#f09a37")

        #ax.plot(filteredBasalData.deliveryTimeHoursRelativeToFirstDelivery,
        #        filteredBasalData.rate, lw=4, ls="--", color="#f09a37")

        # calculate the time relative to the first delivery
        ...
        # TODO: figure out the difference between the nonlinear least sqaures fit and the curve fit

        from scipy.optimize import curve_fit

        #def bgVelocity(x, isf, counterBasalEffect):
        #    return (x * isf) + counterBasalEffect
