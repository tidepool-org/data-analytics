...
# looks like getGlucoseChange is returning the the first and last data values
# startDate is date of first glucose value
# endDate is the date of the last glucse value
# it runs the prediction over the last 30 minutes using the insulin and glucose effects
# lastGlucose = retrospectivePrediction.last
# discrepancy = change.end.quantity.doubleValue(for: glucoseUnit) - lastGlucose.quantity.doubleValue(for: glucoseUnit) // mg/dL
# so the discrepancy is just between the last perdicted value and the last glucose value

# discrepancyTime = max(change.end.endDate.timeIntervalSince(change.start.endDate), settings.retrospectiveCorrectionInterval)

# givent that settings.retrospectiveCorrectionInterval is 30 minutes, it will take whatever is bigger

#  velocity = HKQuantity(unit: velocityUnit, doubleValue: discrepancy / discrepancyTime)
#    let type = HKQuantityType.quantityType(forIdentifier: HKQuantityTypeIdentifier.bloodGlucose)
#    let glucose = quantity: change.end.quantity, start: change.end.startDate, end: change.end.endDate)
#
#    self.retrospectiveGlucoseEffect = glucose.decayEffect(atRate: velocity, for: effectDuration)

# so that velocity is then applied with the decayEffect over the effectDuration, which is 60 minutes

# intercept = rate = velocity
# decayStartDate is the startDate of the simulation + delta = 5 minutes
# slope = -intercept / (duration - delta)  // mg/dL/s/s
# values = [GlucoseEffect(startDate: startDate, quantity: quantity)]
# date = decayStartDate
# lastValue = quantity.doubleValue(for: glucoseUnit)

# it then loops through all of the values and adds (intercept + slope * date.timeIntervalSince(decayStartDate)) * delta
