...
parser.add_argument(
    ...
    default=os.path.abspath(os.path.join(os.path.dirname(__file__), "example-csv.csv")),
    help="csv, xlsx, or json file that contains Tidepool data",
)

parser.add_argument(
    "--deprecated-timezone-list",
    dest="timezoneAliasesFilePathAndName",
    default=os.path.abspath(
        os.path.join(
            os.path.dirname(__file__), "wikipedia-timezone-aliases-2018-04-28.csv"
        )
    ),
...
parser.add_argument(
    ...
    dest="outputPath",
    default=os.path.abspath(
        os.path.join(os.path.dirname(__file__), "output", "dataWithLocalTimeEstimates")
    ),
...
parser.add_argument(
    ...
    default=os.path.abspath(
        os.path.join(os.path.dirname(__file__), "output", "daySeriesData")
    ),
...
def convertDeprecatedTimezoneToAlias(df, tzAlias):
    if "timezone" in df:
        ...
        for uniqueTimezone in uniqueTimezones:
            alias = tzAlias.loc[
                tzAlias.tz.str.endswith(uniqueTimezone), ["alias"]
            ].values
            if len(alias) == 1:
...
def largeTimezoneOffsetCorrection(df):
    ...
    while (df.timezoneOffset > 840).sum() > 0:
        df.loc[df.timezoneOffset > 840, ["conversionOffset"]] = df.loc[
            df.timezoneOffset > 840, ["conversionOffset"]
        ] - (1440 * 60 * 1000)
    ...
    while (df.timezoneOffset < -720).sum() > 0:
        df.loc[df.timezoneOffset < -720, ["conversionOffset"]] = df.loc[
            df.timezoneOffset < -720, ["conversionOffset"]
        ] + (1440 * 60 * 1000)
...
def getAndPreprocessUploadRecords(df):
    ...
    ud.loc[
        ((ud.deviceTags.str.contains("cgm")) & (ud.timeProcessing != "none")),
        ["deviceType"],
    ] = "cgm"
...
def isDSTChangeDay(currentDate, currentTimezone):
    tzoCurrentDay = getTimezoneOffset(pd.to_datetime(currentDate), currentTimezone)
    tzoPreviousDay = getTimezoneOffset(
        pd.to_datetime(currentDate) + timedelta(days=-1), currentTimezone
    )
...
def addDeviceDaySeries(df, dfContDays, deviceTypeName):
    if len(df) > 0:
                    ...
                    for i in dfDaySeries.index:
                        if pd.notnull(dfDaySeries.loc[i, "timezone"]):
                            tzo = getTimezoneOffset(
                                pd.to_datetime(i), dfDaySeries.loc[i, "timezone"]
                            )
        ...
        dfDaySeries = dfDaySeries.add_prefix(deviceTypeName + ".").rename(
            columns={deviceTypeName + ".date": "date"}
        )

        dfContDays = pd.merge(
            dfContDays, dfDaySeries.reset_index(), on="date", how="left"
        )
...
def imputeUploadRecords(df, contDays, deviceTypeName):
    daySeries = addDeviceDaySeries(df, contDays, deviceTypeName)
    ...
    if (len(df) > 0) & (deviceTypeName + ".timezone" in daySeries):
        for i in daySeries.index[1:]:
            if pd.isnull(daySeries[deviceTypeName + ".timezone"][i]):
                daySeries.loc[i, [deviceTypeName + ".timezone"]] = daySeries.loc[
                    i - 1, deviceTypeName + ".timezone"
                ]
            if pd.notnull(daySeries[deviceTypeName + ".timezone"][i]):
                tz = daySeries.loc[i, deviceTypeName + ".timezone"]
                tzo = getTimezoneOffset(pd.to_datetime(daySeries.loc[i, "date"]), tz)
                daySeries.loc[i, deviceTypeName + ".timezoneOffset"] = tzo

            if pd.notnull(daySeries[deviceTypeName + ".timeProcessing"][i - 1]):
                daySeries.loc[i, deviceTypeName + ".timeProcessing"] = daySeries.loc[
                    i - 1, deviceTypeName + ".timeProcessing"
                ]
...
def estimateTzAndTzoWithUploadRecords(cDF):
    ...
    cDF.loc[
        (
            (cDF["est.timezoneOffset"] != cDF["home.imputed.timezoneOffset"])
...
def estimateTzAndTzoWithDeviceRecords(cDF):
    ...
    tzoDiffGT60 = (
        abs(
            cDF.loc[sIndices, "cgm.timezoneOffset"]
            - cDF.loc[sIndices, "pump.timezoneOffset"]
        )
        > 60
    )
...
def getRangeOfTZOsForTimezone(tz):
    minMaxTzo = [
        getTimezoneOffset(pd.to_datetime("1/1/2017"), tz),
...
def assignTzoFromImputedSeries(df, i, imputedSeries):
    ...
    df.loc[i, ["est.timezoneOffset"]] = df.loc[i, imputedSeries + ".timezoneOffset"]

    df.loc[i, ["est.timezone"]] = df.loc[i, imputedSeries + ".timezone"]

    df.loc[i, ["est.timeProcessing"]] = df.loc[i, imputedSeries + ".timeProcessing"]
...
def compareDeviceTzoToImputedSeries(df, sIdx, device):
    for i in sIdx:
        ...
        for imputedSeries in [
            "pump.upload.imputed",
            "cgm.upload.imputed",
            "healthkit.upload.imputed",
            "home.imputed",
        ]:
            ...
            if pd.isnull(df.loc[i, "est.timezone"]):
                ...
                if (
                    ...
                    df = addAnnotation(df, i, "tz-inferred-from-" + imputedSeries)
...
def assignTzoFromPreviousDay(df, i, previousDayTz):
    ...
    df.loc[i, ["est.timezoneOffset"]] = getTimezoneOffset(
        pd.to_datetime(df.loc[i, "date"]), previousDayTz
    )
...
def assignTzoFromDeviceTzo(df, i, device):
    ...
    df.loc[i, ["est.type"]] = "DEVICE"
    df.loc[i, ["est.timezoneOffset"]] = df.loc[i, device + ".timezoneOffset"]
    df.loc[i, ["est.timeProcessing"]] = df.loc[
        i, device + ".upload.imputed.timeProcessing"
    ]
            ...
            if pd.notnull(df.loc[i - 1, "est.timezone"]):
                ...
                # dst range (as that is a common problem with this data)
                    ...
                    # if it is not DST change day, then mark this as uncertain
                        ...
                        # or 45 minutes.
                        if (
                            (df.loc[i, device + ".timezoneOffset"] == min(dstRange))
                            | (df.loc[i, device + ".timezoneOffset"] == max(dstRange))
                        ) & (
                            (df.loc[i - 1, "est.timezoneOffset"] == min(dstRange))
                            | (df.loc[i - 1, "est.timezoneOffset"] == max(dstRange))
                        ):
        ...
        # and if there are no errors, it is likely a travel day
            ...
            if (df.loc[i, device + ".timezoneOffset"] in dstRange) & (
                ...
                if isDSTChangeDay(df.loc[i, "date"], comparisonTz):
                    ...
                    df = addAnnotation(df, i, "dst-change-day")
                    df.loc[i, ["est.type"]] = "DEVICE"
                    df.loc[i, ["est.timezoneOffset"]] = df.loc[
                        i, device + ".timezoneOffset"
                    ]
                    df.loc[i, ["est.timezone"]] = df.loc[i, "home.imputed.timezone"]
                    df.loc[i, ["est.timeProcessing"]] = df.loc[
                        i, device + ".upload.imputed.timeProcessing"
                    ]
                ...
                # if it is not DST change day, then mark this as uncertain
                    ...
                    if (
                        ...
                        (df.loc[i, "home.imputed.timezoneOffset"] == min(dstRange))
                        | (df.loc[i, "home.imputed.timezoneOffset"] == max(dstRange))
                    ):
...
def getImputIndices(df, sIdx, hIdx):
    ...
    nextDayIdx = min(currentDayIdx + min(tempList[tempList >= 0]), lastDayIdx)
...
def imputeByTimezone(df, currentDay, prevDaywData, nextDaywData):
    ...
    if prevDaywData >= 0:
        ...
        if df.loc[prevDaywData, "est.timezone"] == df.loc[nextDaywData, "est.timezone"]:
            ...
            for i in range(currentDay, nextDaywData):
                ...
                df.loc[i, ["est.timezoneOffset"]] = getTimezoneOffset(
                    pd.to_datetime(df.loc[i, "date"]), tz
                )
        ...
        # the same TZO. In that case the gap should be marked as UNCERTAIN
            ...
            for i in range(currentDay, nextDaywData):
                ...
                df.loc[i, ["est.timezoneOffset"]] = df.loc[
                    prevDaywData, "est.timezoneOffset"
                ]
...
def imputeTzAndTzo(cDF):
    ...
    if len(hasTzoIndices) > 0:
        if len(sIndices) > 0:
            ...
            while (sIndices.min() < max(hasTzoIndices)) & (len(sIndices) > 0):

                currentDay, prevDayWithDay, nextDayIdx = getImputIndices(
                    cDF, sIndices, hasTzoIndices
                )

                cDF = imputeByTimezone(cDF, currentDay, prevDayWithDay, nextDayIdx)
                ...
                sIndices = cDF[
                    (
                        ...
                        & (
                            ~cDF["est.annotations"]
                            .str.contains("unable-to-impute-tzo")
                            .fillna(False)
                        )
                    )
                ].index
            ...
            if len(sIndices) > 0:
                ...
                for i in range(currentDay, lastDay + 1):
                    if (
                        cDF.loc[prevDayWithDay, "est.timezoneOffset"]
                        == cDF.loc[prevDayWithDay, "home.imputed.timezoneOffset"]
                    ):

                        cDF.loc[i, ["est.type"]] = "IMPUTE"

                        cDF.loc[i, ["est.timezoneOffset"]] = cDF.loc[
                            i, "home.imputed.timezoneOffset"
                        ]

                        cDF.loc[i, ["est.timezone"]] = cDF.loc[
                            i, "home.imputed.timezone"
                        ]
...
def reorderColumns(cDF):
    cDF = cDF[
        [
            "pump.upload.imputed.timezoneOffset",
...
def getListOfDSTChangeDays(cDF):
    ...
    dstChangeDays = cDF[
        abs(
            ...
            - cDF["home.imputed.timezoneOffset"].shift(-1)
        )
        > 0
    ].date
...
def correctEstimatesAroundDst(df, cDF):
    ...
    for d in dstChangeDays:
        dstIndex = df[
            (df.date > (d + timedelta(days=-2))) & (df.date < (d + timedelta(days=2)))
        ].index
        for dIdx in dstIndex:
            if pd.notnull(df.loc[dIdx, "est.timezone"]):
                ...
                tzoNum = int(
                    tz.localize(
                        df.loc[dIdx, "utcTime"] + timedelta(hours=minHoursToLocal)
                    ).strftime("%z")
                )
                tzoHours = np.floor(tzoNum / 100)
                tzoMinutes = round((tzoNum / 100 - tzoHours) * 100, 0)
                tzoSign = np.sign(tzoHours)
                tzo = int((tzoHours * 60) + (tzoMinutes * tzoSign))
                localTime = df.loc[dIdx, "utcTime"] + pd.to_timedelta(tzo, unit="m")
...
def applyLocalTimeEstimates(df, cDF):
    ...
    df["est.localTime"] = df["utcTime"] + pd.to_timedelta(
        df["est.timezoneOffset"], unit="m"
    )
...
if os.path.isfile(args.timezoneAliasesFilePathAndName):
    timezoneAliases = pd.read_csv(args.timezoneAliasesFilePathAndName, low_memory=False)
else:
    sys.exit("{0} is not a valid file".format(args.timezoneAliasesFilePathAndName))
...
for deviceType in ["pump", "cgm", "healthkit"]:
    ...
    cDays = imputeUploadRecords(tempUploadData, cDays, deviceType + ".upload.imputed")
...
if pd.notnull(args.daySeriesOutputPath):
    cDays.to_csv(
        os.path.join(args.daySeriesOutputPath, daySeriesFileName + "-daySeries.csv")
    )
