...
parser.add_argument(
    ...
    default=os.path.abspath(
        os.path.join(os.path.dirname(__file__), "tidepool-qualification-criteria.json")
    ),
...
def getClosedLoopDays(groupedData, qualCriteria, metadata):
    ...
    if "basal" in groupedData.type.unique():
        ...
        closedLoopDF = pd.DataFrame(
            nTempBasalsPerDay, index=nTempBasalsPerDay.index.date
        )
        closedLoopDF["date"] = nTempBasalsPerDay.index.date
        closedLoopDF["basal.closedLoopDays"] = closedLoopDF["basal.temp.count"] >= nTB
        ...
        med670g = pd.DataFrame(topPump.str.contains("1780")).rename(
            columns={"top": "670g"}
        )
...
def getCalculatorCounts(groupedData, metadata):
    if "wizard" in groupedData.type.unique():
        ...
        calculatorData, nDuplicatesRemoved = removeDuplicates(
            calculatorData, ["time", "bolus"]
        )

        metadata["calculator.duplicatesRemoved.count"] = nDuplicatesRemoved

        # get start and end times
        calculatorBeginDate, calculatorEndDate = getStartAndEndTimes(
...
def dexcomCriteria(df):
    ...
    df.loc[(isClosedLoop | isNonDexcom), ["bolus.count", "cgm.count"]] = 0
...
def isQualifyingDay(df, bolusCriteria, percentCgmCriteria, cgmPoints1Day):
    ...
    df["qualifyAndNotQualifyGroups"] = (
        df.qualifyingDay != df.qualifyingDay.shift()
    ).cumsum()
...
def getSummaryStats(df, dayStatsDF):
    ...
    percentQualifyingDays = round(
        (numberQualifyingDays / numberContiguousDays) * 100, 1
    )
    df["qualifyingDays.percent"] = percentQualifyingDays

    avgBolusCalculations = round(
        dayStatsDF.loc[dayStatsDF.qualifyingDay == 1, "calculator.count"].mean(), 1
    )
...
def getQualifyingTier(
    df,
    criteriaName,
    contDayCriteria,
    avgBolusCalculationsCriteria,
    percQualDayCriteria,
    ...
    for i in range(0, len(df) - (contDayCriteria - 1)):
        ...
        avgBolusCalculationsPerDay = df["calculator.count"].iloc[i:tempIndex].mean()
        tempDF.loc[i, "avgBolusCalculationsPerDay"] = avgBolusCalculationsPerDay
        ...
        gapGroups = df.gapGroup.iloc[i:tempIndex].loc[df.gapGroup > 0].astype(str)
        ...
        tier = (
            numberContiguousDays == contDayCriteria
    ...
    if sum(df[tierName].fillna(0) * 1) > 0:
        ...
        qualifiedEndDate = groupObj.get_group(biggestGroup).date.max() + pd.Timedelta(
            days=contDayCriteria
        )
...
def qualify(df, metaDF, q, idx):
    ...
    for j in range(0, len(q["tierNames"])):
        df, qualifyingResults = getQualifyingTier(
            df,
            q["tierNames"][j],
        ...
        qrDF = pd.DataFrame(qualifyingResults, index=[idx]).add_prefix(
            q["tierNames"][j] + "."
        )

        metaDF = pd.concat([metaDF, qrDF], axis=1)
        if qualifyingResults["qualified"]:
            metaDF[q["tierAbbr"] + ".topTier"] = q["tierNames"][j]
...
donorJsonData = os.path.join(donorFolder, phiDateStamp + "-donorJsonData", "")
if not os.path.isdir(donorJsonData):
    sys.exit("{0} is not a directory".format(donorJsonData))

# create output folder(s)
donorQualifyFolder = os.path.join(donorFolder, args.dateStamp + "-qualified", "")
            ...
            if "upload" in data.type.unique():
                ...
                if "hClosedLoop" in qualCriteria["name"]:
                    if "basal" in data.type.unique():
                        ...
                        bd = data[
                            (data.type == "basal") & (data.deliveryType == "temp")
                        ]
                        tempBasalCounts = pd.DataFrame(
                            bd.groupby("date").deliveryType.count()
                        ).reset_index()
                        tempBasalCounts.rename(
                            {"deliveryType": "tempBasalCounts"}, axis=1, inplace=True
                        )
                        data = pd.merge(data, tempBasalCounts, on="date")
                        data = data[
                            data.tempBasalCounts
                ...
                if ("cbg" in data.type.unique()) and ("bolus" in data.type.unique()):
                    ...
                    metadata[
                        "all.negativeDurationsRemoved.count"
                    ...
                    cgmData, nDuplicatesRemovedDeviceTime = removeCgmDuplicates(
                        cgmData, "deviceTime"
                    )
                    metadata[
                        "cgm.nDuplicatesRemovedDeviceTime.count"
                    ] = nDuplicatesRemovedDeviceTime

                    # get rid of duplicates that have the same ["time", "value"]
                    cgmData, nDuplicatesRemovedUtcTime = removeCgmDuplicates(
                        cgmData, "time"
                    )

                    metadata[
                        "cgm.nDuplicatesRemovedUtcTime.count"
                    ] = nDuplicatesRemovedUtcTime

                    # round time to the nearest 5 minutes
                    cgmData = td.clean.round_time(
                        cgmData,
                    ...
                    cgmData, nDuplicatesRemovedRoundedTime = removeDuplicates(
                        cgmData, "roundedTime"
                    )

                    metadata[
                        "cgm.nDuplicatesRemovedRoundedTime.count"
                    ...
                    cgmRecordsPerDay = pd.DataFrame(catDF.value.count()).rename(
                        columns={"value": "cgm.count"}
                    )
                    dayDate = catDF.dayIndex.describe()["top"]
                    dexcomCGM = catDF.dexcomCGM.describe()["top"]
                    nTypesCGM = catDF.dexcomCGM.describe()["unique"]
                    cgmRecordsPerDay["cgm.dexcomOnly"] = dexcomCGM & (nTypesCGM == 1)
                    ...
                    # get rid of duplicates
                    bolusData, nDuplicatesRemoved = removeDuplicates(
                        bolusData, ["time", "normal"]
                    )
                    ...
                    # get start and end times
                    bolusBeginDate, bolusEndDate = getStartAndEndTimes(
                        bolusData, "dayIndex"
                    )
                    ...
                    bolusRecordsPerDay = pd.DataFrame(catDF.subType.count()).rename(
                        columns={"subType": "bolus.count"}
                    )

                    bolusRecordsPerDay["date"] = bolusRecordsPerDay.index

                    # %% GET CALCULATOR DATA (AKA WIZARD DATA)
                    calculatorRecordsPerDay, metadata = getCalculatorCounts(
                        groupedData, metadata
                    )

                    # %% GET CLOSED LOOP DAYS WITH TEMP BASAL DATA
                    isClosedLoopDay, is670g, metadata = getClosedLoopDays(
                        groupedData, qualCriteria, metadata
                    )
                    ...
                    # merge data
                    contiguousData = pd.merge(
                        contiguousData, bolusRecordsPerDay, on="date", how="left"
                    )
                    contiguousData = pd.merge(
                        contiguousData, cgmRecordsPerDay, on="date", how="left"
                    )
                    contiguousData = pd.merge(
                        contiguousData, calculatorRecordsPerDay, on="date", how="left"
                    )
                    contiguousData = pd.merge(
                        contiguousData, isClosedLoopDay, on="date", how="left"
                    )
                    contiguousData = pd.merge(
                        contiguousData, is670g, on="date", how="left"
                    )

                    # fill in nan's with 0s
                    for dataType in ["bolus", "cgm", "calculator", "basal.temp"]:
                        contiguousData[dataType + ".count"] = contiguousData[
                            dataType + ".count"
                        ].fillna(0)

                    if (
                        (len(contiguousData) > 0)
                        ...
                        contiguousData = isQualifyingDay(
                            contiguousData,
                        ...
                        contiguousData, metadata = qualify(
                            contiguousData, metadata, qualCriteria, dIndex
                        )
                        ...
                        allMetaData = pd.concat(
                            [allMetaData, metadata], axis=0, sort=False
                        )
