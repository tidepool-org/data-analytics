...
parser.add_argument("-q",
                    ...
                    default="../data/dexcom-qualification-criteria.json",
...
csvFileNameAndPath = '../data/PHI-2018-05-06-donor-data/' + \
    'PHI-2018-05-19-localTimeEstimateData/LTE/PHI-1e8446673b.csv'

if os.path.exists(csvFileNameAndPath):
#    phiUserID = "PHI-" + userID
    data = pd.read_csv(csvFileNameAndPath, low_memory=False)
    ...
    if (("cbg" in data.type.unique()) and ("bolus" in data.type.unique())):
        ...
        pdb.set_trace()
        # get rid of dexcom-api data
        if "payload" in data:
            data = data[~data.payload.astype(str).str.contains("systemTime")]
...
# %% SAVE RESULTS
            tier = int(metadata["D.topTier"][0][1:])
            if tier > 0:
                bd = metadata["D" + str(tier) + ".qualified.beginDate"].astype(str).values
                ed = metadata["D" + str(tier) + ".qualified.endDate"].astype(str).values
                print(str(tier) + "-" + bd + "-" +  ed)
            else:
                print("file not qualified")
else:
    print("file would not load")
