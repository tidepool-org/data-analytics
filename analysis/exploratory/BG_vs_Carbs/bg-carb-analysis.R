...
for(file_number in 1:files_to_process){
  ...
  for(idVal in 1:length(unique_uploadId)){
    upload_dates = rbind(upload_dates, as.POSIXct(data$est.localTime[which(data$uploadId==unique_uploadId[idVal])[1]]))
  }
  ...
  for(i in 1:length(days_to_analyze)){
    ...
    all_carb_entries = c(all_carb_entries, carb_data$carbInput[which(carb_data$virtualDay==days_to_analyze[i])])
    carb_timestamps = c(carb_timestamps,as.character(round_date(carb_data$est.localTime[which(carb_data$virtualDay==days_to_analyze[i])],"5 minutes")))
    ...
    big.day = c(big.day, rep(carb_data$day[which(carb_data$virtualDay==days_to_analyze[i])[1]],length(bg_data$value[which(bg_data$virtualDay==days_to_analyze[i])])))
    big.timestamp = c(big.timestamp, as.character(round_date(bg_data$est.localTime[which(bg_data$virtualDay==days_to_analyze[i])],"5 minutes")))
    ...
    big.age = c(big.age, rep(as.integer(round(difftime(days_to_analyze[i],as.Date(metaData$bDay[which(metaData$hashID==files[file_number])]))/365)),length(bg_data$value[which(bg_data$virtualDay==days_to_analyze[i])])))
  }
