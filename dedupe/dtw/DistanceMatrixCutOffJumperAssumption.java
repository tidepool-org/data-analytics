...
public class DistanceMatrixCutOffJumperAssumption
{
...
//QUICK TEST CASES
	...
	// int[] ArrayY = new int[]{0,1,1,2,3,2,1,1,3,1,2};
		...
		//{0,0,1,1,0};//
		   ...
		   int[] ArrayY = new int[]{1,2,1,1,2,4,4,3,7,5}; 
		   int[] ArrayX = new int[]{1,1,1,2,4,4,3,2,9,9,9,9};
	 ...
	 // int[] ArrayY = new int[]{2,2,2,3,2,2};
		...
		int PercentageLongestTraversalCutOff = 20; //percent of the max traversal that is still considered a length possible of producing a match
				...
				DisMatrixFillingDiagonally(DataSetY,DataSetX, PercentageLongestTraversalCutOff);
			...
			//	PrintMatrix(LZScoordinates);
				...
				//System.out.println("8. Percentage of Longest Traversal," + LengthofShorterArray(DataSetY,DataSetX) + " , that we care about finding...."+ PercentageLongestTraversalCutOff + "=" + (PercentageLongestTraversalCutOff)*PercentageLongestTraversalCutOff);
	}	

//public static void ReadGoogleSlides()

	public static int LengthofShorterArray(int[]X,int[]Y)
	{
	...
	public static void DisMatrixFillingDiagonally(int[]DY, int[]DX, int cutoffPercent) //cutoff% is the minimun % of the longest traversal that we care about finding continious matches in
	{//arguemnts are the arrays of data points
		...
		double shortersidelengthD = shortersidelength; //turn int to double
		int CutOffPoint = (int)((shortersidelengthD*(cutoffPercent)/100.0f));//compare two doubles then cast back to an int.
		//lower cut off, less traversals removed, more traversals total
		//(shortersidelength/1); prevents-all traversals are equal to or below the cutoff threshold  MatrixHelper() from running
		...
		for(int i=0; i<(TraversalsStartingonXAxis+1); i++)//goes through as many times are you need to traverse the middle traversal to the bottom left corner.
			{	
				...
				if(JumpingPreChecker(DY, DX, XstartPos, YstartPos, CutOffPoint)==true)
				{
...
//System.out.println(LongerSideY);
		...
		for(int p=0; p<TraversalsStartingonYAxis; p++) //traversals from 1 above the middle to top right corner.
			{
				...
				if(JumpingPreChecker(DY, DX, XstartPos, YstartPos, CutOffPoint)==true) //this method checks the traversal in increments of the CutOffPoint Length, which is to say in quantities that we truely care about.
				{//this jumping prechecker- assumes that we only care about continious strings of zeros rather than total zeros and finding every zero
	...
	public static boolean JumpingPreChecker(int[]DY, int[]DX, int Xpos, int Ypos, int CutOffPoint)
	{//Cut off point =  minimum length of continuous matching values care about.
		...
		if(DX.length>=DY.length)
			shortersidelength = DY.length; //finding max traversal length to set up interval length, aka minimum length of continious zeros to be relavent.
		else
		...
		for(int i =0; i<(shortersidelength/CutOffPoint); i++)
		{
			...
			{
				EnterMatrixHelper = true;
				break;
			}
			Xpos = Xpos + (shortersidelength/CutOffPoint); //increment X/Y positions accordingly
			Ypos = Ypos + (shortersidelength/CutOffPoint);

			if(Xpos >= (DX.length-1) || Ypos >= (DY.length-1)) //prevent exiting the indexing values
				break;
		}

		return EnterMatrixHelper;
